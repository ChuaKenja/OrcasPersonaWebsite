...
const ITEMS = [
  { id: "i", badge: "I", title: "GITHUB", subtitle: "Repositories / Activity", rank: 10 },
  { id: "ii", badge: "II", title: "SKILLS", subtitle: "Languages & Tools", rank: 10 },
  { id: "iii", badge: "III", title: "PROJECTS", subtitle: "Featured Work", rank: 10 },
...
export default function ResumePage({ src }) {
  ...
  useEffect(() => {
    const fetchGitHubData = async () => {
      try {
        const response = await fetch(`https://api.github.com/users/${profile.username}/repos?sort=updated&per_page=4`);
        const data = await response.json();
        const formattedRepos = data
          .filter(repo => !repo.name.toLowerCase().includes('hussain') && !repo.name.toLowerCase().includes('mohammad'))
          .map((repo, idx) => ({
        ...
        {active === 1 && (
          <div className="resume-detail-panel">
            <div className="resume-detail-top">
              ...
              <div className="resume-detail-top-progress">{profile.techStack.languages.length + profile.techStack.tools.length}/∞</div>
            </div>

            <div className="resume-detail-list">
              {profile.techStack.languages.map((lang, idx) => (
            ...
            <div className="resume-detail-bottom">
              ...
              <div className="resume-detail-bullets">
                <div className="resume-detail-bullet">- {profile.bio}</div>
                <div className="resume-detail-bullet">- {profile.quickFacts.find(fact => fact.label === "Approach")?.value}</div>
                <div className="resume-detail-bullet">- {profile.quickFacts.find(fact => fact.label === "Currently")?.value}</div>
              </div>
            </div>
          </div>
        )}
        ...
        {active === 2 && (
          <div className="resume-detail-panel">
            <div className="resume-detail-top">
              ...
              <div className="resume-detail-top-progress">{profile.projects?.length || 1}/∞</div>
            </div>
            ...
            <div className="resume-detail-list">
              {profile.projects && profile.projects.length > 0 ? (
                profile.projects.map((project, idx) => (
                  <div className="resume-detail-row" key={idx}>
                    <div className="resume-detail-row-index">{String(idx + 1).padStart(2, '0')}</div>
                    <div className="resume-detail-row-title">{project.name}</div>
                ...
                <div className="resume-detail-row">
                  ...
                  <div className="resume-detail-row-title">{profile.username} Portfolio</div>
            ...
            <div className="resume-detail-bottom">
              ...
              <div className="resume-detail-bullets">
                <div className="resume-detail-bullet">- {profile.quickFacts.find(fact => fact.label === "Approach")?.value}</div>
                <div className="resume-detail-bullet">- Focus on {profile.stats.find(stat => stat.label === "Primary Focus")?.value.toLowerCase()}</div>
                <div className="resume-detail-bullet">- {profile.intro}</div>
              </div>
            </div>
          </div>
        )}
        ...
        {active === 3 && (
          <div className="resume-detail-panel">
            <div className="resume-detail-top">
              ...
              <div className="resume-detail-top-title">LEARNING</div>
              <div className="resume-detail-top-progress">{profile.learning.length}/∞</div>
            </div>
            ...
            <div className="resume-detail-list">
              {profile.learning.map((item, idx) => (
                <div className="resume-detail-row" key={idx}>
                  <div className="resume-detail-row-index">{String(idx + 1).padStart(2, '0')}</div>
                  <div className="resume-detail-row-title">{item}</div>
                  <div className="resume-detail-status">Active</div>
                </div>
              ))}
            </div>
            ...
            <div className="resume-detail-bottom">
              ...
              <div className="resume-detail-bullets">
                <div className="resume-detail-bullet">- {profile.intro}</div>
                <div className="resume-detail-bullet">- Building foundational knowledge across multiple domains</div>
                <div className="resume-detail-bullet">- {profile.quickFacts.find(fact => fact.label === "Currently")?.value}</div>
              </div>
            </div>
          </div>
